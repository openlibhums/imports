...
class WordPressImportTests(TestCase):
    ...
    def test_download_autogenerated_pdfs(self):
        generator = "http://example.org/wp-content/themes/imaginary/pdf.php?ida={}"
        pages = {
            generator.format(wp_id): b"%PDF-1.4 rendered " + str(wp_id).encode()
            for wp_id in range(201, 209)
        }
        pages[generator.format(209)] = b"<html>Not found</html>"
        # the theme is read from the first article page that can be fetched
        pages["http://example.org/articles/the-mirror-of-clocks/"] = (
            '<a href="http://example.org/articles/editorial-note/">next</a>'
            '<a class="download" href="{}">Download PDF</a>'.format(
                generator.format(201)
            )
        ).encode("utf-8")
        with mock.patch(URLOPEN, fake_urlopen(pages)):
            output = self.run_import("--download-autogenerated-pdfs")

        self.assertIn("theme detected from the site: imaginary", output)
        self.assertIn("autogenerated pdfs added: 7", output)
        self.assertIn("autogenerated pdfs failed: 1", output)
        rivers = self.article(205)
        galley = rivers.galley_set.get(type="pdf")
        self.assertEqual(galley.label, "PDF")
        self.assertEqual(galley.file.original_filename, "on-fictional-rivers.pdf")
        with open(galley.file.self_article_path(), "rb") as handle:
            self.assertEqual(handle.read(), b"%PDF-1.4 rendered 205")
        self.assertFalse(self.article(209).galley_set.filter(type="pdf").exists())
        self.assertTrue(self.article(201).galley_set.filter(type="html").exists())

        # a rerun replaces the galley rather than adding a second one
        with mock.patch(URLOPEN, fake_urlopen(pages)):
            self.run_import("--download-autogenerated-pdfs")
        self.assertEqual(self.article(205).galley_set.filter(type="pdf").count(), 1)

    def test_autogenerated_pdfs_skipped_without_theme(self):
        # no article page can be fetched, so the theme stays unknown: the
        # option is dropped and a warning closes the run
        output = self.run_import("--download-autogenerated-pdfs")
        self.assertIn("warning: autogenerated PDFs were not downloaded", output)
        self.assertIn("autogenerated pdfs added: 0", output)
        self.assertIn("autogenerated pdfs failed: 0", output)
        self.assertFalse(
            core_models.Galley.objects.filter(
                article__journal=self.journal, type="pdf"
            ).exists()
        )
        self.assertEqual(self.articles().count(), 8)

    def test_download_autogenerated_pdfs_with_given_theme_and_real_upload(self):
        pages = {
            "http://example.org/wp-content/themes/given/pdf.php?ida=205": b"%PDF-1.4 site",
            "http://example.org/uploads/rivers.pdf": b"%PDF-1.4 upload",
        }
        with mock.patch(URLOPEN, fake_urlopen(pages)):
            output = self.run_import(
                "--download-pdfs", "--download-autogenerated-pdfs", "--theme", "given"
            )
        self.assertNotIn("theme detected", output)
        labels = list(
            self.article(205)
            .galley_set.filter(type="pdf")
            .values_list("label", flat=True)
        )
        self.assertEqual(sorted(labels), ["PDF", "PDF (autogenerated)"])
