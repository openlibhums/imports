...
def import_ojs3_review_attachments(
    client, journal, ojs_id=None,
    ojs_statuses=None, raise_on_exc=False,
):
    # Only needed if something went wrong and we need to re-import review attachments
    if ojs_id:
    ...
    for d in ojs_articles:
        try:
            logger.info("Importing review attachments: %s", d["id"])
            ojs3_importers.import_review_attachments(
                client, journal, d,
            )
        except Exception as e:
            if raise_on_exc:
