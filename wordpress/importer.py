...
class WordPressImporter:
    def __init__(
        ...
        download_autogenerated_pdfs=False,
        theme=None,
        live_order=False,
        skip_pages=False,
        exclude=None,
        ...
        self.download_autogenerated_pdfs = download_autogenerated_pdfs
        # the WP theme directory name, needed for the autogenerated PDFs;
        # detected from a live article page when not given
        ...
        self.stats = {
            ...
            "authors_created": 0,
            "pdf_galleys_added": 0,
            "autogenerated_pdfs_added": 0,
            "autogenerated_pdfs_failed": 0,
    ...
    def fetch(self, url):
        """Download a URL from the site; None (and a log line) on failure."""
        try:
    ...
    def download(self, url):
        """fetch() with a per-run cache: the same image is embedded by
        many articles."""
        if url not in self.image_cache:
            self.image_cache[url] = self.fetch(url)
    ...
    def add_pdf_galley(self, article, pdf_bytes, name, label):
    ...
    def theme_from_article_page(self, item):
        """The theme directory named by the "download PDF" link of the
        article's live page, or None."""
        wp_id = element_text(item, "wp:post_id")
        link = element_text(item, "link").strip()
        page = self.fetch(link) if link else None
        if page is None:
            return None
        for match in consts.AUTOGENERATED_PDF_LINK_RE.finditer(
            page.decode("utf-8", errors="replace")
        ):
            if match.group(2) != wp_id:
                continue
            theme = consts.THEME_NAME_RE.search(html.unescape(match.group(0)))
            if theme:
                return theme.group(1)
        return None

    def determine_theme(self, articles):
        """Make sure the theme name is known before autogenerated PDFs are
        fetched: given explicitly, or read from the first live article
        pages. Without it the autogenerated PDFs are skipped altogether."""
        if not self.download_autogenerated_pdfs or self.theme:
            return
        candidates = (
            item
            for item in articles
            if element_text(item, "wp:status") == "publish"
            and element_text(item, "link").strip()
        )
        for attempt, item in enumerate(candidates):
            if attempt >= consts.THEME_DETECTION_ATTEMPTS:
                break
            self.theme = self.theme_from_article_page(item)
            if self.theme:
                self.log("  * theme detected from the site: {}".format(self.theme))
                return
        self.download_autogenerated_pdfs = False
        self.warnings.append(
            "autogenerated PDFs were not downloaded: the theme could not be"
            " determined from the site, pass it with --theme"
        )

    def attach_autogenerated_pdf(self, article, wp_id):
        """Store the PDF the WordPress theme autogenerates for the article.

        The site has no stored PDFs: its download link renders the article
        page to PDF on request. Editors nevertheless expect every article to
        offer the same download, so the rendering is snapshotted before the
        site goes away.
        """
        url = self.site_url.rstrip("/") + consts.AUTOGENERATED_PDF_PATH_TEMPLATE.format(
            theme=self.theme, post_id=wp_id
        )
        pdf_bytes = self.fetch(url)
        if pdf_bytes is None or not pdf_bytes.startswith(consts.PDF_MAGIC):
            if pdf_bytes is not None:
                self.log("  ! not a PDF: {}".format(url))
            self.stats["autogenerated_pdfs_failed"] += 1
            return
        label = consts.PDF_GALLEY_LABEL
        if article.galley_set.filter(type="pdf").exists():
            label = "{} (autogenerated)".format(consts.PDF_GALLEY_LABEL)
        name = "{}.pdf".format(slugify(article.title)[:60] or "article")
        self.add_pdf_galley(article, pdf_bytes, name, label)
        self.stats["autogenerated_pdfs_added"] += 1
    ...
    def attach_galleys(self, article, wp_id, meta):
        ...
        if self.download_autogenerated_pdfs:
            self.attach_autogenerated_pdf(article, wp_id)
    ...
    def run(self):
        ...
        for token in sorted(
            ...
            self.warnings.append(
                "filter token '{}' did not match any article".format(token)
            )
