...
# The site's theme generates a PDF of every article on request; there is
# no stored file and the export does not name the theme. Unless the theme
# name is given, it is read from the "download PDF" link of a live article
# page: an href mentioning "pdf" whose query carries the article's WP post
# id, inside the theme's directory.
AUTOGENERATED_PDF_LINK_RE = re.compile(
    r'href="([^"]*pdf[^"]*?[?&]\w+=)(\d+)([^"]*)"', re.I
)
THEME_NAME_RE = re.compile(r"/wp-content/themes/([^/\"?#]+)/")
THEME_DETECTION_ATTEMPTS = 5
AUTOGENERATED_PDF_PATH_TEMPLATE = "/wp-content/themes/{theme}/pdf.php?ida={post_id}"
PDF_MAGIC = b"%PDF"
